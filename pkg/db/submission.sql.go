...
type GetRoundWiseQuestionSubmissionsRow struct {
	RoundID          int32
	QuestionID       uuid.UUID
	SubmissionsCount int64
}

func (q *Queries) GetRoundWiseQuestionSubmissions(ctx context.Context) ([]GetRoundWiseQuestionSubmissionsRow, error) {
	rows, err := q.db.Query(ctx, getRoundWiseQuestionSubmissions)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetRoundWiseQuestionSubmissionsRow
	for rows.Next() {
		var i GetRoundWiseQuestionSubmissionsRow
...
type GetSubmissionsByLanguageRow struct {
	LanguageID       int32
	SubmissionsCount int64
}

func (q *Queries) GetSubmissionsByLanguage(ctx context.Context) ([]GetSubmissionsByLanguageRow, error) {
	rows, err := q.db.Query(ctx, getSubmissionsByLanguage)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetSubmissionsByLanguageRow
	for rows.Next() {
		var i GetSubmissionsByLanguageRow
...
type GetSubmissionsByUserIDRow struct {
	ID              uuid.UUID
	QuestionID      uuid.UUID
	LanguageID      int32
	SourceCode      string
...
func (q *Queries) GetSubmissionsByUserID(ctx context.Context, userID uuid.UUID) ([]GetSubmissionsByUserIDRow, error) {
	rows, err := q.db.Query(ctx, getSubmissionsByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetSubmissionsByUserIDRow
	for rows.Next() {
		var i GetSubmissionsByUserIDRow
		if err := rows.Scan(
			&i.ID,
			&i.QuestionID,
			&i.LanguageID,
